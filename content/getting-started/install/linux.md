...
You must have Go already installed on your machine in order to install TinyGo. We recommend Go v1.23 or above.
...
```shell
wget https://github.com/tinygo-org/tinygo/releases/download/v0.41.0/tinygo_0.41.0_armhf.deb
sudo dpkg -i tinygo_0.41.0_armhf.deb
```
...
```shell
$ tinygo version
tinygo version 0.41.0 linux/amd64 (using go version go1.25.5 and LLVM version 20.1.1)
```
...
Scroll down on that page to the "Artifacts" and click to download the file for your desired architecture, for example `tinygo0.41.0-dev.linux-amd64.tar.gz`.
