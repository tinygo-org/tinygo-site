...
To compile `blinky1.hex` targeting an ARM microcontroller, such as the PCA10040:
    ...
    docker run --rm -v $(pwd):/src tinygo/tinygo:0.22.0 tinygo build -o /src/blinky1.hex -size=short -target=pca10040 examples/blinky1

To compile `blinky1.hex` targeting an AVR microcontroller such as the Arduino:

    docker run --rm -v $(pwd):/src tinygo/tinygo:0.22.0 tinygo build -o /src/blinky1.hex -size=short -target=arduino examples/blinky1
...
into the docker image for those dependencies to be found:
    ...
    docker run -v $GOPATH:/go -e "GOPATH=/go" tinygo/tinygo:0.22.0 tinygo build -o /go/src/github.com/myuser/myrepo/wasm.wasm -target wasm --no-debug /go/src/github.com/myuser/myrepo/wasm-main.go
